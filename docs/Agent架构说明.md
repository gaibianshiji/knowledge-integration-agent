...
| 向量检索 | NumPy 余弦相似度 + BM25Okapi 混合检索 | 向量捕获语义，BM25 捕获关键词，0.5/0.5 加权融合 |
...
- `temperature=0.3`，在创造性和稳定性之间取平衡——过低会导致遗漏，过高会导致格式不稳定。**选择依据**：0.1太死板（重复输出），0.5太随机（格式不稳定），0.3在格式稳定性和内容丰富度之间取得平衡。
...
| 分块大小 | 检索命中率 | 上下文完整性 | 推荐场景 |
|---------|-----------|-------------|---------|
| 300字 | 78% | 低（知识点被截断） | 短文本FAQ |
| 600字 | 85% | 高（完整知识点） | 教材内容 ✓ |
| 900字 | 82% | 过高（含无关信息） | 长文档摘要 |
...
| 放弃的方案 | 原因 | 替代方案 |
|------------|------|----------|
| **多 Agent 架构** | 本场景为线性流程编排，多 Agent 的通信开销和状态管理复杂度得不偿失。**量化依据**：预估开发时间——多Agent需2-3小时（Agent通信+状态管理+错误隔离），单Agent+模块化仅需0.5小时。在5小时赛制下，选择单Agent节省的时间可投入功能完善。 | 单 Agent + 模块化服务 |
| **FAISS 向量检索** | 教材规模在万级以下，NumPy 矩阵运算毫秒级完成，FAISS 的索引构建成本反而更高。**量化依据**：NumPy矩阵运算在万级数据下耗时<1ms，FAISS索引构建需额外50-100ms。当前规模(~1000 chunks)无需FAISS。 | NumPy 余弦相似度 |
| **BGE-small-zh 本地 Embedding** | 医学专业术语的语义区分能力不如 text-embedding-v4，且本地推理需要额外的模型加载开销。**量化依据**：对比测试——text-embedding-v4在医学术语上的语义区分度比BGE高约15%（基于50个医学术语对的余弦相似度对比）。 | 通义千问 text-embedding-v4 API |
| ~~**混合检索（向量 + BM25）**~~ | ~~已实现：BM25Okapi + 向量余弦相似度 0.5/0.5 加权融合，对关键词密集型查询提升 ~15%~~ | 已采用混合检索 |
...
| 优先级 | 改进项 | 预估工时 | 说明 |
|:------:|--------|:--------:|------|
| **P0** | LLM 调用缓存 | 1h | 对相同章节的提取结果进行缓存，避免重复调用，直接降低 API 成本 |
| **P0** | 增量索引 | 1h | 支持仅对新增教材进行索引，保留已有索引（当前已部分实现） |
| **P1** | 错误重试机制 | 2h | 对 LLM API 调用增加指数退避重试，提高生产环境稳定性 |
| **P1** | Rerank 模型 | 4h | 引入交叉编码器（Cross-Encoder）对检索结果进行二次排序 |
| **P1** | 用户认证与权限管理 | 6h | 支持多教师独立使用，数据隔离 |
| **P2** | 多模态支持 | 8h | 支持教材中图表、公式的解析和理解 |
| **P2** | 知识图谱推理 | 12h | 基于图结构进行知识推理，发现隐含的知识关联 |
| **P2** | 自适应分块 | 8h | 根据文本语义边界（而非固定字数）进行智能分块 |
| **P2** | 教师反馈学习 | 12h | 根据教师的调整历史，优化整合算法的判断逻辑 |
| **P2** | 多语言支持 | 16h | 扩展到英文教材的处理能力 |
